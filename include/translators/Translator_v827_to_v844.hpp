#pragma once
// ─────────────────────────────────────────────────────────────────────────────
// Translator: 1.21.10x (proto 827) → 1.21.11x (proto 844)
// Source: GlacieTeam/ProtocolLib releases table (авторитетный источник)
//
// КАК ЗАПОЛНЯТЬ:
//   1. Найди changelog между 1.21.10x и 1.21.11x на minecraft.wiki или
...
class Translator_v827_to_v844 final : public IVersionTranslator {
public:
    ...
    [[nodiscard]] std::unique_ptr<IPacket> translate(
        ...
        // ══════════════════════════════════════════════════════════════════════
        // TODO: Вставить реальные case-ы после изучения diff 1.21.10x→1.21.11x
        //
        // ШАБЛОН для поля, добавленного в 1.21.11x:
        //
        ...
        //         // Клиент 1.21.10x не знает mNewField — обнуляем/убираем
        //         sg->mNewFieldAddedIn844 = {};
        //     } else {
        //         // Сервер 1.21.11x ожидает mNewField — проставляем дефолт
        //         // (уже нулевой, но явно)
        //     }
        //     return packet;
        // }
        //
        // ШАБЛОН для нового пакета, появившегося в 1.21.11x:
        //
        // case MinecraftPacketIds::SomeNewPacket: {
        //     if (direction == TranslateDirection::Clientbound) {
        //         return nullptr; // Клиент 1.21.10x не знает этот пакет — дроп
        //     }
        //     // Serverbound: такой пакет клиент 1.21.10x послать не может —
